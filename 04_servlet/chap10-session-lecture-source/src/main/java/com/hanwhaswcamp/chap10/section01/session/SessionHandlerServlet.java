...
public class SessionHandlerServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 세션은 브라우저 당 한 개의 고유 아이디를 가지고  하나의 인스턴스르 이용한다.
        /* 반복 요청 시 동일 session id를 반환한다.
        * 단, 브라우저가 종료 되면 해당 세션이 졸효 되므로 그 이후의 요청이 다른 session id를 반환한다.
        * session id는 cookie로 보관되고 있다. */
        System.out.println(session.getId());

        System.out.println(session.getMaxInactiveInterval()); // 세션의 기본 유지 시간은 30분
        session.setMaxInactiveInterval(60*10);      // 필요에 따라 늘이거나 줄일 수 있다 (10 분)
